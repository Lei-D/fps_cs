...
//'
// [[Rcpp::export]]
List fps(NumericMatrix S, double ndim, double covPercent, int nsol = 50, 
  ...
  for(int i = 0; i < nsol; i++) {
    ...
    niter[i] = admm(FantopeProjection(ndim),  
                    ...
                    maxiter, tolerance_abs, covPercent);
  ...
  // modified by Lei
  // prepare for return right value for selected.feature
  int nCov;
  nCov = ceil(active.size() * covPercent);
  active = active.subvec(0, nCov-1);
  // end of modification
